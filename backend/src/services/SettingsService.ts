...
export async function updateAllowNewUserRegistration(allowed: boolean) {
    await repositories.updateSetting("allow_new_user_registration", allowed ? 'true' : 'false');
...
export async function getAllSettings() {
    ...
    const settingsObj = allSettings.reduce((acc: Record<string, string | null>, setting: { key: string; value: string | null }) => {
        if (setting.key) {
